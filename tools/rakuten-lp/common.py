...
（既定: tools/rakuten-lp/secrets.env。git管理外＝ネットに公開されない）から読み込む。
"""
...
def redact(text: str) -> str:
    ...
    for name in ("RAKUTEN_APPLICATION_ID", "RAKUTEN_ACCESS_KEY", "GEMINI_API_KEY", "OPENAI_API_KEY", "ANTHROPIC_API_KEY", "LINE_CHANNEL_ACCESS_TOKEN"):
        v = os.getenv(name)
        if v and len(v) >= 6:
            text = text.replace(v, "***")
    return text
