...
def current_ip() -> str:
    """このパソコンがインターネットに出ていくときのIPアドレス(IPv4)を調べる（楽天の許可IP登録用）。"""
...
def _get(url: str, params: dict, interval: float) -> list:
    ...
    for attempt in range(3):
        ...
        if resp.status_code in (429, 500, 502, 503):
            ...
            continue
        if "CLIENT_IP_NOT_ALLOWED" in resp.text:
            raise RakutenAPIError(f"このパソコンのIPアドレス（{current_ip()}）が楽天ウェブサービスに登録されていません。"
                                  "マニュアル「IPアドレスが変わったとき」の手順で登録し直してください")
