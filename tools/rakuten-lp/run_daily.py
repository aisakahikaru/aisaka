...
使い方（キットのフォルダで。ふだんは「3_本番実行.bat」などのファイルをダブルクリックすればOK）:
...
def page_ctx(config, p, slug, created, modified, fetched=None):
    ...
    return {"site_url": site, "site_name": config["site_name"], "root_rel": "../../../",
            ...
            "published": created, "modified": modified, "favicon": listing.favicon_for(config), "logo": listing.logo_of(config)}
...
OUR_PATHS = ["."]  # サイト全体を公開（APIキーの secrets.env と logs は .gitignore で除外＋公開前に二重チェック）
SECRET_FILES = ("secrets.env", ".env")
BRANCH = "main"


def unrelated_changes() -> list:
    """（キット版）サイト全体を公開するため、公開を止める対象はなし。"""
    return []
...
def publish(n_new: int) -> None:
    ...
    if any(line.strip().endswith(SECRET_FILES) for line in staged.splitlines()):
        git("reset", check=False)
        raise RuntimeError("APIキーのファイル(secrets.env)が公開されそうになったため中止しました。.gitignore を確認してください")
    if not staged.strip():
        log("公開する変更はありません")
        return
...
def notify_line(text: str) -> None:
    """結果をLINEに通知する（任意）。secrets.env に LINE_CHANNEL_ACCESS_TOKEN と LINE_USER_ID があるときだけ送る。
    LINE公式アカウント(Messaging API)の「プッシュメッセージ」を使う。未設定なら何もしない。"""
    token, to = os.getenv("LINE_CHANNEL_ACCESS_TOKEN"), os.getenv("LINE_USER_ID")
    if not (token and to):
        return
    try:
        import requests
        r = requests.post("https://api.line.me/v2/bot/message/push", timeout=20,
                          headers={"Authorization": f"Bearer {token}", "Content-Type": "application/json"},
                          json={"to": to, "messages": [{"type": "text", "text": text[:4900]}]})
        if r.status_code != 200:
            log(f"LINE通知に失敗（HTTP {r.status_code}）")
    except Exception as e:
        log(f"LINE通知に失敗（{type(e).__name__}）")
...
def main() -> int:
    ...
    try:
        log(f"文章作成AI: {gemini.set_provider(config)}")
...
def budget_exhausted(config: dict, db: dict) -> bool:
    ...
    if used + 4 > config.get("gemini_daily_calls_limit", 450):
        log(f"本日のAI利用上限（{config.get('gemini_daily_calls_limit')}回）に近づいたため、今回はここまで")
        return True
    return False
...
def run(config, args) -> int:
    ...
    try:
        ...
        import site_pages
        site_pages.build_all(config)  # トップページの「新着の商品ガイド」を更新
    except Exception as e:
        log(f"トップページ更新に失敗: {e}")
    try:
    ...
    log(f"今回の新規LP: {len(new_recs)}本 / 関連リンク更新: {n_rel}本 / AI({gemini.label()})呼び出し: {gemini.calls_used()}回")
    ...
    if args.notify:
        ...
        notify_line(f"✅ 楽天LP自動生成\n本日 {done}本（今回 {len(new_recs)}本・{gemini.label()} {gemini.calls_used()}回）\n"
