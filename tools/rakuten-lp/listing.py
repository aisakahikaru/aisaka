...
def logo_of(config: dict) -> str:
    """ヘッダーの四角いロゴに表示する文字（config.json の logo_text。未設定ならサイト名の1文字目）。"""
    return (config.get("logo_text") or config.get("site_name") or "S").strip()[:2]


def favicon_for(config: dict) -> str:
    """ブラウザのタブに表示する小さなアイコン（ロゴ文字＋テーマカラーのSVG）。"""
    color = config.get("theme_color") or "#2563eb"
    svg = ("<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 64 64'><rect width='64' height='64' rx='14' fill='" + color + "'/>"
           "<text x='32' y='44' font-family='Arial,Helvetica,sans-serif' font-size='34' font-weight='700' fill='#ffffff' "
           "text-anchor='middle'>" + logo_of(config)[:1] + "</text></svg>")
...
def _page(title, desc, canonical, root, h1, lead, body, site_name, logo="S", favicon=FAVICON):
...
<a class="site-logo" href="{root}index.html"><span class="site-logo-mark" aria-hidden="true">{e(logo)}</span>{e(site_name)}</a>
...
<p class="site-logo"><span class="site-logo-mark" aria-hidden="true">{e(logo)}</span>{e(site_name)}</p>
...
def build_listings(db: dict, config: dict, ensure: list = ()) -> list:
    ...
    for slug, items in by_cat.items():
        ...
        html_text = _page(f"{label}の商品ガイド", f"{label}カテゴリの商品について、特徴・使う場面・購入前に確認したい点を商品情報をもとに整理したページの一覧です。",
                          ...
                          f"{label}の商品を、楽天市場の商品情報をもとに1商品ずつ整理しています（{len(items)}件）。", body, name, logo_of(config), favicon_for(config))
    ...
    html_text = _page("商品ガイド一覧", "楽天市場の商品について、どんな人に向いているか・特徴・購入前に確認したい点を1商品ずつ整理した商品ガイドの一覧です。",
                      ...
                      "楽天市場の商品情報をもとに、「自分に合う商品か」を判断するための情報を1商品ずつ整理しています。", body, name, logo_of(config), favicon_for(config))
